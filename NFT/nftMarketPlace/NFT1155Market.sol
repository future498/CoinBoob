...
contract NFT1155Market is ReentrancyGuard {
    ...
    IERC20 public _boob; //replace with address of the token you want to accept as payment method
    IERC20 public BNB; // address on mainnet : IERC20(0xbb4CdB9CBd36B01bD1cBaEBF2De08d9173bc095c);
    ...
    // --- Init --- constructor based on that ? should be like that, cause governance = msg.sender at the beginning the owner of the contract 
    function initialize(
        address payable tipsFeeWallet,
        //uint256 minDurationTime,
        uint256 tipsFeeRate,
        uint256 baseRate,
        address BOOBaddress, //Boob BEP20 token of the platform 
        address BNBaddress,
    ) public {
        ...
        //by default we add BNB and $BOOB  
        _supportCurrency[BOOBaddress] = true;
        _supportCurrency[BNBaddress] = true;
    ...
    function addSupportCurrency(address erc20) public onlyGovernance {
        require(_supportCurrency[erc20] == false, "the currency have support already");
        ...
        payable 
    {
        ...
        if (obj.currency == address(0x0)) { //if currency == token (so replace 0x0 by address of your "native" token)
        ...
        // if the buyer pay with another currency -- the buyer/seller should have appropriate wallet !!
        else {
        ...
        obj.buyers.push(msg.sender); //we add msg.sender to list of buyers of the obj
    ...
    function settokenAddress(address addr) external onlyGovernance validAddress(addr) {
        _token = IERC20(addr);
    }
